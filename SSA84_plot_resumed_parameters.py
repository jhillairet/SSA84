...
with plt.style.context('seaborn-whitegrid'):
    fig, ax = plt.subplots()
    df.query('pulse_length > 0.1 and JR_max_log < -2.63').plot(kind='scatter', ax=ax, x='pulse_length', y='V_mean',  s=30, c='JR_max_log', cmap='viridis', vmax=-2.60, alpha=0.6)
    ax.set_xscale('log')
